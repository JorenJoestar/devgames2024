...
layout (std430, set=1, binding=2) buffer DebugGpuFontBuffer {
...
layout (std430, set=1, binding=3) buffer DebugGpuFontEntries {
    DebugGPUStringEntry entries[];
};

layout(std430, set=1, binding=4) buffer DebugGPUFontDispatch {
    uvec4               dispatches[];
};

layout(std430, set=1, binding=5) buffer DebugGPUIndirect {
    uint            vertex_count;
    uint            instance_count;
    uint            first_vertex;
    uint            first_instance;
