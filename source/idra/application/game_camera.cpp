...
void GameCamera::update( InputSystem* input, u32 window_width, u32 window_height, f32 delta_time ) {
    ...
    if ( input->is_mouse_dragging( MOUSE_BUTTONS_RIGHT ) && !ImGui::IsAnyItemHovered() ) {
        ...
        if ( ignore_dragging_frames == 0 ) {
            target_yaw -= ( input->get_mouse_position_x() * 1.f - roundu32(window_width / 2.f) ) * mouse_sensitivity * delta_time;
            target_pitch -= ( input->get_mouse_position_y() * 1.f - roundu32(window_height / 2.f) ) * mouse_sensitivity * delta_time;
        } else {
