...
void data_buffer_get_current( const DataBuffer& data_buffer, float& value ) {
    uint32_t current_index = data_buffer.current_entries - 1;
    data_buffer_get( data_buffer, current_index, value );
}
...
void lexer_next_token( Lexer* lexer, Token& token ) {
    ...
    token.text.data = lexer->position;
    token.text.size = 1;
    ...
    switch ( c ) {
        ...
        case '"':
        {
            ...
            token.text.size = lexer->position - token.text.data;
            if ( lexer->position[0] == '"' ) {
                ++lexer->position;
            }
        } break;
...
bool lexer_expect_keyword( const StringRef& text, uint32_t length, const char* expected_keyword ) {
    ...
    return memcmp( text.data, expected_keyword, length ) == 0;
}

void lexer_goto_line( Lexer* lexer, i32 line ) {
    if ( line < 0 ) {
        //hprintf( "Lexer: trying to go to negative line %d\n", line );
        return;
    }

    while ( lexer->line < (u32)line && lexer->position ) {
...
void lexer_next_line( Lexer* lexer ) {
    ...
    // Arrive until just before the end of the line
    while ( !is_end_of_line( lexer->position[ 0 ] ) && !is_end_of_text( lexer->position[ 0 ] ) ) {
        ++lexer->position;

        //// Handle different End of Line formats
        //if ( *lexer->position == '\r' ) {
        //    ++lexer->position;
        //    if ( *lexer->position == '\n' )
        //        ++lexer->position;
    ...
    // Advance further if \r is present
    if ( *lexer->position == '\r' ) {
        ++lexer->position;
    }

    // Found end of line character, skip it and advance line.
    if ( *lexer->position == '\n' ) {
        ++lexer->position;
    }
