...
namespace idra {
#if defined (IDRA_MATH_OVERFLOW_CHECK)
...
#define hy_math_convert(val, max, type, func)\
    ...
    if (abs(value_container) > max)\
        ilog_warn( "Overflow converting values %llu, %llu\n", value_container, max );\
...
T numerics_convert_ceilf( T value, M max ) {
    i64 value_container = ( i64 )ceilf( value );
    if ( abs( value_container ) > max ) {
        ilog_warn( "Overflow converting values %llu, %llu\n", value_container, max );
    }
            
    const T v = ( T )value_container;
    return v;
}
...
#endif // IDRA_MATH_OVERFLOW_CHECK
