...
#include "gpu/vulkan_forward_declarations.hpp"
#endif // IDRA_VULKAN
...
namespace TextureFormat {
    ...
    inline bool                     is_depth_stencil( TextureFormat::Enum value ) {
        return value == D32_FLOAT_S8X24_UINT || value == D24_UNORM_S8_UINT;
    }
    inline bool                     is_depth_only( TextureFormat::Enum value ) {
        return value >= D32_FLOAT && value < S8_UINT;
    }
    inline bool                     is_stencil_only( TextureFormat::Enum value ) {
        return value == S8_UINT;
    }

    inline bool                     has_depth( TextureFormat::Enum value ) {
        return value >= D32_FLOAT_S8X24_UINT && value < S8_UINT;
    }
    inline bool                     has_stencil( TextureFormat::Enum value ) {
        return value == D32_FLOAT_S8X24_UINT || value == D24_UNORM_S8_UINT || value == S8_UINT;
    }
    inline bool                     has_depth_or_stencil( TextureFormat::Enum value ) {
        return value >= D32_FLOAT_S8X24_UINT && value <= S8_UINT;
    }
...
namespace BufferUsage {
    enum Enum {
        Vertex, Index, Constant, Indirect, Structured, Staging, Count
    };

    enum Mask {
        Vertex_mask = 1 << 0, Index_mask = 1 << 1, Constant_mask = 1 << 2, Indirect_mask = 1 << 3, Structured_mask = 1 << 4, Staging_mask = 1 << 5, Count_mask = 1 << 6
    };
...
namespace ShaderStage {
    ...
    enum Mask {
        Vertex_mask = 1 << 0, Fragment_mask = 1 << 1, Compute_mask = 1 << 2,
        RayGen_mask = 1 << 3, Intersect_mask = 1 << 4, AnyHit_mask = 1 << 5,
        Closest_mask = 1 << 6, Miss_mask = 1 << 7, Callable_mask = 1 << 8,
        Task_mask = 1 << 9, Mesh_mask = 1 << 10, Count_mask = 1 << 11
    };
...
namespace QueueType {
    ...
    enum Mask {
        Graphics_mask = 1 << 0, Compute_mask = 1 << 1, Transfer_mask = 1 << 2, Count_mask = 1 << 3
    };
...
namespace DescriptorType {
    ...
    enum Mask {
        Sampler_mask = 1 << 0, Texture_mask = 1 << 1, Image_mask = 1 << 2, 
        Constants_mask = 1 << 3, StructuredBuffer_mask = 1 << 4, 
        AccelerationStructure_mask = 1 << 5, Count_mask = 1 << 6
    };
...
namespace PipelineType {
    ...
    enum Enum {
        Graphics, Compute, Raytracing, Count
    };
} // namespace PipelineType
...
namespace SwapchainStatus {

    enum Enum {
        Ready, NotReady, Resized, Count
    }; // enum Enum
